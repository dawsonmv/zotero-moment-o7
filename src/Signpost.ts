...
export class Signpost {
	...
	private static parseSignpostingLinks(linkHeader: string): AuthorInfo[] {
		...
		const authorLinks = links.filter(
			link =>
				link.rel === 'author' ||
	...
	private static extractFromHTML(html: string): AuthorInfo[] {
		...
		const metaTags = doc.querySelectorAll(
			'meta[name="author"], meta[property="author"], meta[name="DC.creator"], meta[name="citation_author"]'
		);
		...
		const orcidMetas = doc.querySelectorAll(
			'meta[name="citation_author_orcid"], meta[property="citation_author_orcid"]'
		);
			...
			for (const creator of creators) {
				if (creator.creatorType === 'author' && creator.lastName) {
					...
					const matchingAuthor = authors.find(
						a => a.name && this.fuzzyMatchNames(`${creator.firstName} ${creator.lastName}`, a.name)
					);
