...
export class CredentialManager {
  ...
  private initWarningShown = false;
  ...
  private getProfileIdentifier(): string {
    ...
    try {
      ...
      const profileDir =
        zotero.Profile?.dir || zotero.DataDirectory?.dir || "default-profile";
  ...
  async storeCredential(key: string, value: string): Promise<void> {
    ...
    // Fallback: basic obfuscation (base64) - NOT SECURE
    // This is only marginally better than plaintext and should not be relied upon
  ...
  async migrateIfNeeded(): Promise<void> {
    ...
    for (const key of credentialKeys) {
      ...
      if (
        ...
        !stored.startsWith(this.KEY_PREFIX)
      ) {
