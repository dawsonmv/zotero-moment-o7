...
export class HealthChecker {
	...
	getSystemHealth(): {
		...
		const healthyCount = healths.filter(h => h.status === HealthStatus.HEALTHY).length;
		const degradedCount = healths.filter(h => h.status === HealthStatus.DEGRADED).length;
		const unhealthyCount = healths.filter(h => h.status === HealthStatus.UNHEALTHY).length;
	...
	generateReport(): string {
		...
		for (const service of services) {
			...
			lines.push(`    Success Rate: ${(service.successRate * 100).toFixed(1)}%`);
