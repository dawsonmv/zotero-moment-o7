...
export class Logger {
	...
	notice(category: string, message: string, context?: Record<string, unknown>): void {
		this.log(LogLevel.NOTICE, category, message, context);
	}

	warning(category: string, message: string, context?: Record<string, unknown>): void {
...
export const createArchiveLogger = (): CategoryLogger => Logger.getInstance().child('Archive');

export const createMementoLogger = (): CategoryLogger => Logger.getInstance().child('Memento');
