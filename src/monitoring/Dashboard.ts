...
export class Dashboard {
	...
	generateReport(periodMs: number = 86400000): MonitoringReport {
		...
		return {
			...
			summary: {
				...
				successRate: stats.archiveAttempts > 0 ? stats.archiveSuccesses / stats.archiveAttempts : 0,
	...
	getQuickStatus(): {
		...
		if (systemHealth.unhealthyCount > 0) {
			...
			details.push(
				`Unhealthy services: ${systemHealth.unhealthyCount}/${systemHealth.healthyCount + systemHealth.degradedCount + systemHealth.unhealthyCount}`
			);
		}
	...
	formatReportAsText(report: MonitoringReport): string {
		...
		for (const service of report.serviceDetails) {
			lines.push(
				`${statusIcon(service.status)} ${service.serviceId.padEnd(20)} ${service.status.padEnd(10)} ${(service.successRate * 100).toFixed(0)}%`
			);
		}
