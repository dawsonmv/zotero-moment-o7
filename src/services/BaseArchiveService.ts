...
<pre>${this.escapeHtml(robustLinkHTML)}</pre>`;
	...
	protected mapHttpError(error: any): ArchiveError {
		...
		switch (status) {
			...
			case 404:
				return new ArchiveError(ArchiveErrorType.NotFound, 'The URL could not be found.', 404);
			case 523:
				return new ArchiveError(
					ArchiveErrorType.Blocked,
			...
			default:
				...
				if (error.message?.includes('timeout')) {
					return new ArchiveError(ArchiveErrorType.Timeout, 'Archive request timed out.', 0);
				}
				return new ArchiveError(
