...
export class Histogram {
  ...
  constructor(
    public readonly name: string,
    public readonly bucketBoundaries: number[] = [
...
export class MetricsRegistry {
  ...
  getAggregatedStats(periodMs: number): AggregatedStats {
    ...
    for (const archive of periodArchives) {
      ...
      svc.avgLatency =
        (svc.avgLatency * (svc.attempts - 1) + archive.duration) / svc.attempts;
    }
  ...
  getServiceMetrics(serviceId: string): {
    ...
    return {
      ...
      successRate:
        serviceArchives.length > 0 ? successes / serviceArchives.length : 0,
      avgDuration:
        serviceArchives.length > 0 ? totalDuration / serviceArchives.length : 0,
