...
Zotero.MomentoW8.PermaCCService = class extends Zotero.MomentoW8.BaseArchiveService {
	constructor() {
		super({
			name: "Perma.cc",
			id: "permacc",
	...
	async isAvailable() {
		...
		const apiKey = Zotero.Prefs.get("extensions.zotero.momentow8.permaccApiKey", true);
		if (!apiKey) {
			return false;
		}
		...
		try {
			...
			Zotero.debug(`MomentoW8: Error checking Perma.cc availability: ${e}`);
			return false;
		}
	}
	...
	getApiKey() {
		let apiKey = Zotero.Prefs.get("extensions.zotero.momentow8.permaccApiKey", true);
		...
		if (!apiKey) {
			...
			if (result && input.value) {
				...
				Zotero.Prefs.set("extensions.zotero.momentow8.permaccApiKey", apiKey, true);
			}
		}
	...
	async archiveItem(item) {
		...
		try {
			...
			if (error.status === 401) {
				...
				Zotero.Prefs.clear("extensions.zotero.momentow8.permaccApiKey", true);
	...
	async getOrCreateFolder(folderName) {
		...
		try {
			...
			Zotero.debug(`MomentoW8: Error managing Perma.cc folders: ${e}`);
		}
