...
Zotero.MomentoW8.RobustLinkCreator = {
	/**
	...
	async createRobustLinks(items) {
		...
		const robustServicesStr = Zotero.Prefs.get("extensions.zotero.momentow8.robustLinkServices", true) || "internetarchive,archivetoday";
		...
		for (const item of items) {
			...
			for (const serviceId of robustServices) {
				try {
					const service = Zotero.MomentoW8.ServiceRegistry.get(serviceId);
					if (!service) {
						continue;
					}
