...
Zotero.MomentoW8.ArchiveCoordinator = {
	init: function () {
		this.mementoChecker = Zotero.MomentoW8.MementoChecker;
		this.registry = Zotero.MomentoW8.ServiceRegistry;
	},
	...
	async archiveItems(items, serviceId = null) {
		...
		for (const item of items) {
			try {
				...
				Zotero.debug(`MomentoW8: Error archiving item ${item.id}: ${e}`);
				results.push({
					item: item,
					success: false,
					error: e.message
	...
	async checkExistingArchives(url) {
		try {
			...
			Zotero.debug(`MomentoW8: Error checking existing archives: ${e}`);
		}
		return [];
	},
	...
	async archiveWithBestService(item, existingArchives) {
		...
		const fallbackOrderStr = Zotero.Prefs.get("extensions.zotero.momentow8.fallbackOrder", true) || "internetarchive,archivetoday,arquivopt,permacc,ukwebarchive";
	...
	async autoArchive(item) {
		...
		const defaultService = Zotero.Prefs.get("extensions.zotero.momentow8.defaultService", true) || "internetarchive";

		try {
			return await this.archiveItem(item, defaultService);
		} catch (e) {
			Zotero.debug(`MomentoW8: Auto-archive failed for item ${item.id}: ${e}`);
			return null;
		}
	},
