...
import { RobustLinkCreator } from '../src/RobustLinkCreator';
...
jest.mock('../src/preferences/PreferencesManager', () => ({
	PreferencesManager: {
		getEnabledServices: jest.fn().mockReturnValue(['internetarchive', 'archivetoday', 'permacc']),
		getFallbackOrder: jest.fn().mockReturnValue(['internetarchive', 'archivetoday', 'permacc']),
	},
}));

describe('RobustLinkCreator', () => {
	beforeEach(() => {
		jest.clearAllMocks();
	...
	describe('create', () => {
		it('should create robust link with archive URLs', () => {
			const html = RobustLinkCreator.create({
				originalUrl: 'https://example.com/article',
				archiveUrls: {
					internetarchive: 'https://web.archive.org/web/20231215/https://example.com/article',
				},
		...
		it('should include multiple archive links', () => {
			const html = RobustLinkCreator.create({
		...
		it('should use primary archive based on fallback order', () => {
			(PreferencesManager.getFallbackOrder as jest.Mock).mockReturnValue([
				'archivetoday',
				'internetarchive',
			]);
		...
		it('should return simple link when no archives available', () => {
			(PreferencesManager.getEnabledServices as jest.Mock).mockReturnValue([]);
		...
		it('should use URL as title when title not provided', () => {
			const html = RobustLinkCreator.create({
				originalUrl: 'https://example.com',
				archiveUrls: {
					internetarchive: 'https://web.archive.org/web/123/example.com',
		...
		it('should escape HTML in URLs and titles', () => {
			const html = RobustLinkCreator.create({
		...
		it('should filter archive URLs to enabled services only', () => {
			(PreferencesManager.getEnabledServices as jest.Mock).mockReturnValue(['internetarchive']);
	...
	describe('createSimple', () => {
		it('should create simple robust link', () => {
	...
	describe('service abbreviations', () => {
		it('should use correct abbreviations for services', () => {
