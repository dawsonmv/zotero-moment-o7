...
describe('Tracer', () => {
	...
	describe('startTrace', () => {
		...
		it('should set trace ID on logger', () => {
	...
	describe('startSpan', () => {
		...
		it('should use current trace ID', () => {
			const parentSpan = tracer.startTrace('parent');
	...
	describe('SpanBuilder', () => {
		...
		describe('success', () => {
			it('should mark span as success', () => {
				const span = tracer.startTrace('test');
				const result = span.success();
		...
		describe('error', () => {
			...
			it('should capture stack trace', () => {
				...
				expect(result.tags['error.stack'].split('\n').length).toBeLessThanOrEqual(3);
			});
		});

		describe('getSpanId/getTraceId', () => {
	...
	describe('recordSpan', () => {
		...
		it('should trim spans when over limit', () => {
			// Record many spans
	...
	describe('trace', () => {
		...
		it('should mark span as success on completion', async () => {
		...
		it('should set parent span ID from options', async () => {
			const parentSpan = tracer.startTrace('parent');
	...
	describe('getTrace', () => {
		it('should return all spans for a trace', () => {
			...
			const grandchildSpan = tracer.startSpan('grandchild', childSpan.getSpanId());
	...
	describe('getTraceSummary', () => {
		it('should return summary for trace', () => {
			const rootSpan = tracer.startTrace('root-operation');
			const childSpan = tracer.startSpan('child-operation', rootSpan.getSpanId());
		...
		it('should report error status when any span fails', () => {
			const rootSpan = tracer.startTrace('root');
		...
		it('should calculate span depths', () => {
			const rootSpan = tracer.startTrace('root');
			const childSpan = tracer.startSpan('child', rootSpan.getSpanId());
			const grandchildSpan = tracer.startSpan('grandchild', childSpan.getSpanId());
			...
			const rootDepth = summary?.spans.find(s => s.operation === 'root')?.depth;
			const childDepth = summary?.spans.find(s => s.operation === 'child')?.depth;
			const grandchildDepth = summary?.spans.find(s => s.operation === 'grandchild')?.depth;
	...
	describe('getSlowSpans', () => {
		it('should return spans above threshold', () => {
	...
	describe('export', () => {
		it('should export all spans as JSON', () => {
