...
import { Tracer } from '../../src/monitoring/Tracer';
...
jest.mock('../../src/monitoring/Tracer');
...
describe('Dashboard', () => {
	...
	describe('getDashboardData', () => {
		it('should aggregate all monitoring data', () => {
		...
		it('should include service health', () => {
			const data = dashboard.getDashboardData();
		...
		it('should include recent archives', () => {
			const data = dashboard.getDashboardData();
		...
		it('should include system health info', () => {
			const data = dashboard.getDashboardData();
	...
	describe('generateReport', () => {
		...
		it('should identify most used service', () => {
			const report = dashboard.generateReport();
	...
	describe('generateRecommendations', () => {
		it('should recommend on low success rate', () => {
		...
		it('should recommend on high latency', () => {
			mockMetrics.getSnapshot.mockReturnValue({
				archiveAttempts: 100,
				successRate: 0.9,
			...
			expect(report.recommendations.some(r => r.includes('30 seconds'))).toBe(true);
		});

		it('should recommend on unhealthy services', () => {
			...
			expect(report.recommendations.some(r => r.includes('unhealthy'))).toBe(true);
		});

		it('should recommend on rate limiting', () => {
			mockMetrics.getSnapshot.mockReturnValue({
				archiveAttempts: 100,
				successRate: 0.9,
				avgDuration: 5000,
			...
			expect(report.recommendations.some(r => r.includes('Rate limiting'))).toBe(true);
		});

		it('should recommend on timeout errors', () => {
			mockMetrics.getSnapshot.mockReturnValue({
				archiveAttempts: 100,
				successRate: 0.9,
				avgDuration: 5000,
			...
			expect(report.recommendations.some(r => r.includes('Timeout'))).toBe(true);
		});

		it('should recommend on blocked URLs', () => {
			mockMetrics.getSnapshot.mockReturnValue({
				archiveAttempts: 100,
				successRate: 0.9,
				avgDuration: 5000,
			...
			expect(report.recommendations.some(r => r.includes('blocked'))).toBe(true);
		});

		it('should recommend when no activity', () => {
			...
			expect(report.recommendations.some(r => r.includes('No archive activity'))).toBe(true);
		});

		it('should warn on Perma.cc quota approach', () => {
			mockMetrics.getAggregatedStats.mockReturnValue({
				period: 'day',
				startTime: Date.now() - 86400000,
				endTime: Date.now(),
			...
			expect(report.recommendations.some(r => r.includes('Perma.cc'))).toBe(true);
		});
	});

	describe('getQuickStatus', () => {
		it('should return ok when healthy', () => {
		...
		it('should return error on critical alerts', () => {
			mockAlerting.getActiveAlerts.mockReturnValue([
				{
		...
		it('should return degraded on warnings', () => {
			mockAlerting.getActiveAlerts.mockReturnValue([
				{
		...
		it('should return degraded on unhealthy services', () => {
			mockAlerting.getActiveAlerts.mockReturnValue([]);
			mockHealthChecker.getSystemHealth.mockReturnValue({
				status: HealthStatus.DEGRADED,
				healthyCount: 2,
		...
		it('should return error on high failure rate', () => {
			mockAlerting.getActiveAlerts.mockReturnValue([]);
			mockHealthChecker.getSystemHealth.mockReturnValue({
				status: HealthStatus.HEALTHY,
				healthyCount: 3,
		...
		it('should return degraded on reduced success rate', () => {
			mockAlerting.getActiveAlerts.mockReturnValue([]);
			mockHealthChecker.getSystemHealth.mockReturnValue({
				status: HealthStatus.HEALTHY,
				healthyCount: 3,
	...
	describe('formatReportAsText', () => {
		...
		it('should include alerts section when present', () => {
			const report = dashboard.generateReport();
			const text = dashboard.formatReportAsText(report);
		...
		it('should truncate long alert lists', () => {
	...
	describe('exportAll', () => {
		it('should export all monitoring data as JSON', () => {
		...
		it('should call export on all components', () => {
			dashboard.exportAll();
