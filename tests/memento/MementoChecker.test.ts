...
import { MementoChecker } from '../../src/memento/MementoChecker';

describe('MementoChecker', () => {
	beforeEach(() => {
		jest.clearAllMocks();
	});

	describe('checkUrl', () => {
		it('should find mementos via aggregator', async () => {
			const timeMapResponse = JSON.stringify({
				original_uri: 'https://example.com',
				timegate_uri: 'https://timetravel.mementoweb.org/timegate/https://example.com',
				mementos: {
					list: [
						{
		...
		it('should return empty when no mementos found', async () => {
			(Zotero.HTTP.request as jest.Mock).mockResolvedValue({
				status: 200,
		...
		it('should try multiple aggregators', async () => {
		...
		it('should detect Internet Archive service from URL', async () => {
			(Zotero.HTTP.request as jest.Mock).mockResolvedValue({
				status: 200,
				responseText: JSON.stringify({
					original_uri: 'https://example.com',
		...
		it('should detect Archive.today service from URL', async () => {
			(Zotero.HTTP.request as jest.Mock).mockResolvedValue({
				status: 200,
				responseText: JSON.stringify({
					original_uri: 'https://example.com',
	...
	describe('checkArchive', () => {
		it('should check specific archive for memento', async () => {
			(Zotero.HTTP.request as jest.Mock).mockResolvedValue({
				status: 200,
				responseText: JSON.stringify({
					original_uri: 'https://example.com',
			...
			const result = await MementoChecker.checkArchive('https://example.com', 'Internet Archive');
		...
		it('should return null for unknown archive', async () => {
			const result = await MementoChecker.checkArchive('https://example.com', 'Unknown Archive');

			expect(result).toBeNull();
		});

		it('should return null for Archive.today (no memento support)', async () => {
			const result = await MementoChecker.checkArchive('https://example.com', 'Archive.today');

			expect(result).toBeNull();
		});

		it('should return null when no mementos found', async () => {
			(Zotero.HTTP.request as jest.Mock).mockResolvedValue({
				status: 200,
				responseText: JSON.stringify({
					original_uri: 'https://example.com',
	...
	describe('findExistingMementos', () => {
		...
		it('should find mementos in Extra field', () => {
		...
		it('should find multiple mementos in Extra field', () => {
			(mockItem.getField as jest.Mock).mockImplementation((field: string) => {
				if (field === 'extra') {
					return `Internet Archive: https://web.archive.org/web/20231215/https://example.com
Perma.cc: https://perma.cc/ABC-123`;
		...
		it('should find mementos in notes with robust links', () => {
			const mockNote = {
		...
		it('should return empty array when no mementos found', () => {
			(mockItem.getField as jest.Mock).mockReturnValue('');
		...
		it('should deduplicate mementos', () => {
			(mockItem.getField as jest.Mock).mockImplementation((field: string) => {
				if (field === 'extra') {
...
Internet Archive: https://web.archive.org/web/20231215/https://example.com`;
		...
		it('should handle items without getNotes method', () => {
		...
		it('should detect service from Archive.today URL', () => {
			(mockItem.getField as jest.Mock).mockImplementation((field: string) => {
				if (field === 'extra') {
		...
		it('should detect service from Arquivo.pt URL', () => {
			(mockItem.getField as jest.Mock).mockImplementation((field: string) => {
				if (field === 'extra') {
